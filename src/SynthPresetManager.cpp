...
SynthPresetManager::SynthPresetManager() {
    ...
    for (unsigned long i= 0; i < tableSize; i++) {
        ...
        *sineSynthData++ = (temp*0.19);
        ...
        temp = sinf(phase) + (sinf(phase*2.133)*0.3) + (sinf(phase*3.977)*0.3) + (sinf(phase*6.1)*0.2) + (sinf(phase*8)*0.2);
        ...
        *sineSynthData2++ = temp;
        ...
        temp = (((i *norm)*2-1)*0.5) + (*(sineSynth.dataPointer()+i)*1.4) ;
        
        *tableSawData++ = temp;
        
      
        temp = (  ((i*norm)*2-1)*0.7) + (sinf(phase)) + (sinf(phase*3)*0.4) + (sinf(phase*5)*0.4);
        ...
        *tableSawData2++ = temp;
        ...
        temp = (*(tableNoiseSimple.dataPointer()+i));
...
void SynthPresetManager::createSynth(int preset_,ofxTonicSynth& groupSynth_, Generator& output_, RampedValue& freq_, RampedValue& vol_, ControlGenerator& trigger_, Generator *sineA_,  Generator *sineB_) {
    ...
    if (preset_ == 0) {
        ...
        attack = 0.32;
        ADSR adsr = ADSR(attack, 0.15, 0.45, 0.12).doesSustain(true).legato(true).trigger(trigger_).exponential(0);
        ADSR adsr2 = ADSR(0.05, 0.1, 0.28, 0.20).doesSustain(false).legato(true).trigger(trigger_).exponential(1);
    ...
    //2. simple squarewave_______________________________________________
        ...
        attack = 0.035;
        ADSR adsr = ADSR(attack, 0.04, 0.65, 0.15).doesSustain(false).legato(true).trigger(trigger_);
        ADSR adsr2 = ADSR(0.15, 0.05, 0.16, 0.35).doesSustain(false).legato(true).trigger(trigger_);
        ...
        output_  = ((myTable*adsr) + (myTable2*adsr2))*vol_;
    ...
    //6. snare_______________________________________________
        ...
        ADSR adsr = ADSR(attack, 0.05, 0.41, 0.003).doesSustain(false).legato(true).trigger(trigger_).exponential(0);
        ADSR adsr2 = ADSR(0.0002, 0.10, 0.5, 0.009).doesSustain(false).legato(true).trigger(trigger_);
        ...
        output_ = ((snareOsc*adsr) + (snareOsc2*adsr2))*vol_;
    }
    ...
    //bell test
        ...
        attack = 0.0005;
        ADSR adsr1 = ADSR(attack, 0.08, 0.8, 1.0).doesSustain(false).legato(true).trigger(trigger_);
        ADSR adsr2 = ADSR(0.04, 0.08, 0.5, 0.75).doesSustain(false).legato(true).trigger(trigger_);
